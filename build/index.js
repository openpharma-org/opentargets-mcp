    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            if (!method || !isValidMethod(method)) {
                throw new McpError(ErrorCode.InvalidParams, 'method parameter is required and must be one of: search_targets, search_diseases, get_target_disease_associations, get_disease_targets_summary, get_target_details, get_disease_details');
            }
            switch (method) {
                case 'search_targets':
            ...
            associatedDiseases(page: {index: $offset, size: $size}) {
              ...
              rows {
                ...
                const filteredRows = minScore > 0
                    ? allRows.filter((row) => row.score >= minScore)
                    : allRows;
